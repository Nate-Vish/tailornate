...
Design decisions (Nathan, 2026-07-11): month = calendar with category-colored stripes per day (not plain intensity), zoomable month→week→day and back, drag-to-reschedule between days; planner PROPOSES and user approves (never auto); "busy day" = calendar events AND task load, both weighted.
16. [ ] Real day/week/month views: day=thin today, week=7-day agenda w/ per-day load, month=calendar where each day cell shows CATEGORY-COLORED STRIPES (one thin bar per category that has tasks that day, cap ~3 + "+N"); busyness reads from stripe count. Click-to-zoom month→week→day + back. Drag a task from an open day onto another day cell to reschedule (dnd-kit; drop = set dueDate to that day; manual placement is respected by the planner). Undated tasks live in a 'flexible' lane and get a date by drag or by the planner. — files: TodayView split into DayView/WeekView/MonthView, MonthGrid, lib/tasks/load.ts — verify: stripes match categories (Nathan's 13/14/15 example), drag moves a task between days and updates both cells, three genuinely different layouts
